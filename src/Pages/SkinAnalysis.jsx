...
export default function SkinAnalysis() {
  ...
  const section2 = [
    { label: "Water intake", value: data?.water?.score || 50 },
    { label: "Diet efficiency", value: data?.diet?.score || 50 },
  ];
  ...
  const renderProgressGroup = (data, startDelay = 0) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      {data.map((item, idx) => (
        ...
        >
          <RadialProgress value={isVisible ? item.value : 0} delay={startDelay + idx * 200} />
        ...
        <div className="mt-10 flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/3 space-y-4 text-center lg:text-left">
            <div className="flex justify-center lg:justify-start">
              <img src={SkinAnalysisPageIMG || "/placeholder.svg"} alt="avatar" className="w-20 h-20 mb-2" />
            </div>
            <div className="text-2xl font-semibold">Normal Type</div>
            <p className="text-gray-600 text-sm sm:text-lg">
              Congratulations! You are a happy owner of a normal skin type. Even if you might have some skin concerns like wrinkles or dark circles, your skin is perfect! Your skincare routine should be focused on maintaining your skin beauty and tackling your specific concerns that are quite doable with the right ingredients. Protect the microbiome (natural skin's barrier) as it does a great job in keeping your skin well-balanced. Right diet and lifestyle will support your skin's natural beauty, correct skincare routine will prolong your youth and fortify your skin even more.
            </p>
          </div>
          ...
          <div className="lg:w-2/3">
            ...
            <div className="absolute bottom-10 right-8 md:right-14 md:bottom-7 lg:bottom-10 lg:right-20 cursor-pointer">
              <RowButton text="Let's get started" onClick={() => navigate("/SubscriptionPlans")} />
            </div>
          </div>
        </div>
      </div>
